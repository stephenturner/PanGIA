...
mkdir -p merged_sam
parallel -j$THREAD --halt 1 'printf "@SQ\\tSN:"%s"\\tLN:"%s"\\n" `printf {/.}` `printf {/.} | cut -d"|" -f2` > merged_sam/{};\
  cat sam_*/{} >> merged_sam/{};\
...
def seqReverseComplement( seq ):
    ...
    for base in seq:
        if base not in 'ACGTURYSWKMBDHVNacgturyswkmbdhvn':
            print("[ERROR] base: {0} NOT a DNA sequence".format(base))
            return None
    ...
    for rank in sorted( major_ranks, key=major_ranks.__getitem__ ):
        ...
        for tid in sorted( taxas, key=taxas.__getitem__, reverse=True ):
            ...
            if "S_SA_CL" in res_rollup[tid]:
                #sys.stderr.write("check  res_rollup for taxid\n")
                #sys.stderr.write(str(res_rollup[tid]["S_SA_CL"])+ " " + str(tid))
                #sys.stderr.write("\n")
                #sys.stderr.write(type(res_rollup[tid]["S_SA_CL"]).__name__+"\n")

                #sys.stderr.write("check taxid2type\n")
...
def readMapping(reads, dbs, threads, add_options, seed_length, min_score, samfile, logfile, readmapper,illuminaminimap2):
    ...
    for db in dbs:
        ...
        # TODO: Using minimap2 on short reads requires --secondary=yes option (modified code to include this and made extra arg to pass info about minimap2 illumina, this is default option). Therefore,
        #       the non-primary alignments (0x100) shouldn't be filtered (Not done yet). Also, the SAM 
...
def eval(tax):
    ...
    if str(tax) in spedict.keys():
        ...
        msamfile = pd.read_csv(samfile,sep='\t',header=None,usecols=[0,1,2]) 
...
if __name__ == '__main__':
    ...
    if argvs.mode == 'class':
        ...
        del [res_rollup, out_fp, argvs.relAbu, argvs.mode, argvs.scoreMethod, argvs.minScore, argvs.minRsnb, argvs.minReads, argvs.minLen, argvs.minDc, argvs.displayAll] #DAVE ADDITION FOR MEMORY MANAGEMENT
...
#DAVE'S ADDITIONAL MARKER ANALYSIS
    if argvs.markers:
        ...
        print(samfile)
        msamfile = pd.read_csv(samfile,sep='\t',header=None,usecols=[0,1,2]) #pull in sam file for marker analysis #don't conflict with existing samfile variable
        print(msamfile)
        name = samfile.split('.sam')[0]
        
        #add headers to sam file
        #get headers to add:
        headers = list(set(list(pd.read_csv(samfile,sep='\t',header=None,usecols=[2])[2])))
